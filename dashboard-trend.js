...
'.trend-toolbar h3{font-size:15px;font-weight:700;margin-bottom:12px}',
'.trend-batch-panel{display:flex;flex-direction:column;gap:12px}',
'.trend-batch-row{display:flex;gap:12px;flex-wrap:wrap}',
'.trend-batch-row .tf-group{display:flex;flex-direction:column;gap:3px}',
'.trend-batch-row .tf-group label{font-size:11px;color:var(--muted);font-weight:600}',
'.trend-batch-row select,.trend-batch-row input{padding:5px 8px;border:1px solid #d1d5db;border-radius:6px;font-size:12px}',
'.trend-batch-row select{min-width:140px}',
'.trend-metric-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:4px;max-height:180px;overflow-y:auto;border:1px solid #e5e7eb;border-radius:6px;padding:8px;background:#fafbfc}',
'.trend-metric-grid label{display:flex;align-items:center;gap:4px;font-size:11px;cursor:pointer;padding:2px 0}',
'.trend-metric-grid label:hover{color:#2563eb}',
'.trend-org-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:4px;max-height:120px;overflow-y:auto;border:1px solid #e5e7eb;border-radius:6px;padding:8px;background:#fafbfc}',
'.trend-org-grid label{display:flex;align-items:center;gap:4px;font-size:11px;cursor:pointer;padding:2px 0}',
...
function renderCards(){
  ...
  // 行2：对比机构（checkbox网格）
  h += '<div class="tf-group"><label>对比机构（勾选）</label><div class="trend-org-grid">';
  allOrgs.forEach(function(o){ h += '<label><input type="checkbox" value="'+o+'" class="batch-compare-cb"> '+o+'</label>'; });
  h += '</div></div>';
  
  // 行3：指标（checkbox网格）
  h += '<div class="tf-group"><label>分析指标（勾选）</label><div class="trend-metric-grid">';
  Object.keys(fieldGroups).forEach(function(g){
    h += '<div style="font-weight:600;font-size:11px;color:#64748b;margin-top:4px;padding-bottom:2px;border-bottom:1px solid #e5e7eb">'+g+'</div>';
    fieldGroups[g].forEach(function(f){ h += '<label><input type="checkbox" value="'+f.k+'" class="batch-metric-cb"> '+f.l+'</label>'; });
  });
  h += '</div></div>';
...
window.Trend = {
  ...
  generate: function(){
    ...
    var compareBranches = [];
    var cbs = document.querySelectorAll(".batch-compare-cb:checked");
    cbs.forEach(function(cb){ if(cb.value !== branch) compareBranches.push(cb.value); });
    var metrics = [];
    var mbs = document.querySelectorAll(".batch-metric-cb:checked");
    mbs.forEach(function(cb){ metrics.push(cb.value); });
    if(metrics.length === 0){ alert("请至少勾选一个分析指标"); return; }
